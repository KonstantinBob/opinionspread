...
def findInfinitePeak(dataSet,debug):
    ...
    if debug:
        print 'sign of a = ',str(signA)
             
    # do fit
    summary = fit(xScale,y,err,signA)      
    ...
    if debug:                   
        ...
        plt.savefig('Fit_'+str(sys.argv[1])+'_'+str(size)+'_'+str(p)+'_'+str(q)+'.pdf',bbox_inches = 'tight')
        plt.title('$p='+str(p)+'$ $q='+str(q)+'$')
        plt.savefig('Fit_'+str(sys.argv[1])+'_'+str(size)+'_'+str(p)+'_'+str(q)+'.png',bbox_inches = 'tight')
        plt.close()
